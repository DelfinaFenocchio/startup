...
const otherCast = [new Actor('Paul Winfield', 50), new Actor('Michael Biehn', 50), new Actor('Linda Hamilton', 50)];
terminator.addCast(arnold);
terminator.addCast(otherCast);
const sinEscapeMovie = new Movie("Sin escape", 2015, "1h43min");
const lbfMovie = new Movie("London bajo fuego", 2016, "1h38min");
sinEscapeMovie.on('Pause', function () {
  console.log(`Pausing the movie ${sinEscapeMovie.title}`);
});
sinEscapeMovie.on('Resume', function () {
  console.log(`Resuming the movie ${sinEscapeMovie.title}`);
});
sinEscapeMovie.on('Play', function () {
  console.log(`Playing the movie ${sinEscapeMovie.title}`);
});
sinEscapeMovie.play();
